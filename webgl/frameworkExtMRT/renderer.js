...
var Renderer = function(canvas) {
    ...
    var gl = (function(canvas) {
        ...
        for (var i=0; i<validContextNames.length; i++) {
            ...
            if (context = canvas.getContext(validContextNames[i], ctxAttribs)) {
                var extensions = context.getSupportedExtensions().toString().replace(/,/g, ", ");
                console.log("Found '" + validContextNames[i] + "' context with following extensions:\n" + extensions);
                ...
                DRAW_BUFFERS = context.getExtension("WEBGL_draw_buffers");
                if (DRAW_BUFFERS) {
                    var param = context.getParameter(DRAW_BUFFERS.MAX_COLOR_ATTACHMENTS_WEBGL);
                    console.log(param + " MRTs available.");
                }
    ...
    function renderObject(obj, sp, projMat) {
        ...
        // for rendering to MRTs, if possible
        gl.uniform1f(sp.showOverlay, effectsType);
    ...
    function initFBO(w, h, type, numMrt) {
        ...
        if (DRAW_BUFFERS && numMrt > 1) {
            gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + 1, gl.TEXTURE_2D, tex1, 0);
        }
        ...
        return { fbo: fbo, rbo: rb, tex0: tex, tex1: tex1, width: w, height: h };
    }
    ...
    function initOverlay() {
        ...
        var res = 1; // 1 / 10;
        foreground.renderTarget = initFBO(canvas.width*res, canvas.height*res, type, 2);
        foreground.texture[0]   = foreground.renderTarget.tex0;
        if (foreground.renderTarget.tex1)
            foreground.texture[2] = foreground.renderTarget.tex1;
        ...
        res = 512;
        foreground.shadowTarget = initFBO(res, res, type, 1);
        foreground.texture[1]   = foreground.shadowTarget.tex0;
    ...
    return {
        ...
        drawScene: function() {
            ...
            if (showOverlay) {
                ...
                if (DRAW_BUFFERS) {
                    DRAW_BUFFERS.drawBuffersWEBGL([
                        DRAW_BUFFERS.COLOR_ATTACHMENT0_WEBGL,
                        DRAW_BUFFERS.COLOR_ATTACHMENT1_WEBGL
                    ]);
                }
        ...
        setEffectsMode: function(val) {
            if (val == 4 && !DRAW_BUFFERS) {
                alert("NO WEBGL_draw_buffers available, can't do DoF!");
            }
            else {
                effectsType = val;
                showOverlay = (val > 0);
        ...
        tick: function(stats) {
            ...
            needRender = (effectsType == 1); // false except for wave effect
